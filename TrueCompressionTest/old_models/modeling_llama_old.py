        ...
        if use_cache:
            if self.compress_config is not None:
                if past_key_value is None:
                    ...
                    past_vaule_union = CompressUnion(compress_kwargs=self.compress_config)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig,compress_config = None, layer_idx = 0):
        ...
        self.hidden_size = config.hidden_size
        self.self_attn = (
            LlamaAttention(config=config, compress_config = compress_config, layer_idx = layer_idx)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig,compress_config = None):
        ...
        self.layers = nn.ModuleList([LlamaDecoderLayer(config,compress_config= compress_config,layer_idx = _) for _ in range(config.num_hidden_layers)])
