...
class CompressUnion():
    ...
    def compress_function(self,input_tensor:torch.Tensor):
        ...
        if self.compress_mode == "uniform":
            ...
            output,shape,min,step,values,indices = compress_function[self.compress_mode](input_tensor,self.quantize_bit,self.left)
            self.quantize_part = output
            self.min = min
            self.step = step
            self.shape = shape
            ...
            output,shape,min,step,values,indices,p_base,q_base = compress_function[self.compress_mode](input_tensor,self.quantize_bit,self.left,self.rank,self.loop)
            self.quantize_part = output
            self.min = min
            self.step = step
            self.shape = shape
            ...
            quantized_input, shape, min, step, p_base, q_base,shape_p,shape_q,min_p,min_q,scale_p,scale_q = compress_function[self.compress_mode](input_tensor,self.quantize_bit,self.rank,self.loop)
    ...
    def decompress_function(self):
        if self.compress_mode == "uniform":
            output = decompress_function[self.compress_mode](self.quantize_part,self.quantize_bit,self.shape,self.min,self.step,self.dtype)
        elif self.compress_mode == "outlier":
            output = decompress_function[self.compress_mode](self.quantize_part,self.quantize_bit,self.shape,self.min,self.step,self.dtype,self.values,self.indices)
        elif self.compress_mode == "gear":
