...
parser.add_argument(
    "--task", type=str, default="all", choices=["all", "multiple_choice", "free_form"]
)
...
def run_tasks(tasks, mode, model_index):
    for task in tqdm(tasks):
        ...
        print_first = False
        with open(
            "outputs/test_%s_%s.txt" % (model_index.replace("-", "_"), task), "w"
        ) as fd:
...
def main(
    ...
    if run_multiple_choice:
        run_tasks(
            multiple_choice_tasks, mode="multiple_choice", model_index=model_index
        )
    if run_free_form:
