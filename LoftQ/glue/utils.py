...
class LinearQuantLoRA(nn.Module):
    ...
    def initialize_weight(
        self, quant_weight, left_weight, right_weight, sparse_weight=None, bias=None
    ):
        self.quant.weight = nn.Parameter(
            quant_weight, requires_grad=False
        )  # Freeze the backbone
        ...
        if self.has_lora_adapter:
            lora_A_weight = nn.Parameter(
                self.quant.weight.new_zeros((self.reduced_rank, self.in_feature)),
                requires_grad=True,
            )
            lora_B_weight = nn.Parameter(
                self.quant.weight.new_zeros(
                    (self.out_feature, self.reduced_rank), requires_grad=True
                )
            )
...
def quant_first_iter(weight, L, R, reduced_rank, int_bit, quant_method, **kwargs):
    low_rank_product = L @ R if torch.is_tensor(L) else 0
    residual = weight - low_rank_product
    quant_w = weight_quant_fn(residual, num_bits=int_bit, quant_method=quant_method)
...
def replace_module(
    ...
    **kwargs,
    ...
    for attr_str in dir(module):
        ...
        if (
            args.quant_embedding
            and type(target_attr) == nn.Embedding
            and "token" not in attr_str
        ):
            print("====================================================")
            ...
            if decomposition:
                ...
                for i in range(args.num_iter):
                    weights, L, R, quant_w, final_residual = quant_first_iter(
                        weights, L, R, reduced_rank, int_bit, quant_method, **kwargs
                    )
                if args.num_iter == 0:
                    quant_w = weight_quant_fn(
                        weights, num_bits=int_bit, quant_method=quant_method
                    )

                new_embedding = LinearQuantEmbedding(
                    target_attr.weight.shape[0],
                    target_attr.weight.shape[1],
                    r=int(reduced_rank),
                    args=args,
                )
...
class LinearQuantEmbedding(nn.Embedding, LoRALayer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.has_svd_adapter:
            after_left = F.embedding(x, self.left)
