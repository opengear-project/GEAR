...
def quant_nf4_block(weight, block_size=64, num_bits=2):
    ...
    weight_resize = weight.resize(
        weight.shape[0] * weight.shape[1] // block_size, block_size
    )
...
def quant_uniform(input, num_bits=2, clip_val=None):
    ...
    s = 2**num_bits - 1
    quant_input = torch.round(input_normalized * s).div(
        s
    )  # map to int between 0 and s(2**num_bits-1)
