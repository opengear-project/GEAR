...
from CompressUtils.TrueCompressFunction import (
...
(
    ...
    scale_p,
    scale_q,
) = true_gear_tokenwiseQ_compress(rand_input, 4, rank, loop)
...
rand_input = torch.rand(bsz, numhead, seqlen, dim).to(torch.float16).to("cuda:0")
start = time.time()
quantized_input, shape, min, step, p_base, q_base = true_gear_tokenwiseQ_compress_nopq(
    rand_input, 4, rank, loop
)
torch.cuda.synchronize()
end = time.time()
print("tokenwiseQ-gear compress time:", end - start)
start = time.time()
dequantized_input = true_gear_tokenwiseQ_decompress_nopq(
    quantized_input, 4, shape, min, step, p_base, q_base, torch.float16
)
torch.cuda.synchronize()
end = time.time()
