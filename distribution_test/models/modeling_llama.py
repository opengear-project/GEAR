    ...
    def forward(
        ...
        if past_key_value is not None:
            if (
                ...
                # print("past_key:",past_key.dtype,"key_states_before:",key_states.dtype)
                # print("keynan:",torch.isnan(past_key).any())
                key_states = torch.cat([past_key, key_states], dim=2)
                value_states = torch.cat([past_value, value_states], dim=2)
                # print("key_states:",key_states.dtype)
        ...
        attn_weights_save = attn_weights.clone().cpu()
        ...
        torch.save(
            attn_weights_save, "attn_weights_save_untune" + str(self.layer_id) + ".pt"
        )
        if (
            self.compress_config is not None
            and self.compress_config.compress_method[self.layer_id] == "H2O"
